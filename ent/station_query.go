...
type StationQuery struct {
	...
	withStationSchedules *ScheduleQuery
...
// QueryStationSchedules chains the current query on the "station_schedules" edge.
func (sq *StationQuery) QueryStationSchedules() *ScheduleQuery {
	query := (&ScheduleClient{config: sq.config}).Query()
...
func (sq *StationQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (sq *StationQuery) FirstIDX(ctx context.Context) int64 {
...
func (sq *StationQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (sq *StationQuery) OnlyIDX(ctx context.Context) int64 {
...
func (sq *StationQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (sq *StationQuery) IDsX(ctx context.Context) []int64 {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *StationQuery) Clone() *StationQuery {
	...
	return &StationQuery{
		...
		withStationSchedules: sq.withStationSchedules.Clone(),
...
// WithStationSchedules tells the query-builder to eager-load the nodes that are connected to
// the "station_schedules" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *StationQuery) WithStationSchedules(opts ...func(*ScheduleQuery)) *StationQuery {
	query := (&ScheduleClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withStationSchedules = query
	return sq
}
...
func (sq *StationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Station, error) {
	...
	if query := sq.withStationSchedules; query != nil {
		if err := sq.loadStationSchedules(ctx, query, nodes,
			func(n *Station) { n.Edges.StationSchedules = []*Schedule{} },
			func(n *Station, e *Schedule) { n.Edges.StationSchedules = append(n.Edges.StationSchedules, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StationQuery) loadStationSchedules(ctx context.Context, query *ScheduleQuery, nodes []*Station, init func(*Station), assign func(*Station, *Schedule)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Station)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(schedule.FieldStationID)
	}
	query.Where(predicate.Schedule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(station.StationSchedulesColumn), fks...))
	}))
...
func (sq *StationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(station.Table, station.Columns, sqlgraph.NewFieldSpec(station.FieldID, field.TypeInt64))
	_spec.From = sq.sql
